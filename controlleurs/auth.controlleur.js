...
const generateOtp = async () => {
  const code = Math.floor(1000 + Math.random() * 9000).toString(); // Générer un code OTP à 4 chiffres
  try {
    const newOtp = await prisma.otp.create({ data: { code } });
    // res.status(200).json({ message: 'Message envoyé !', newOtp });
    return newOtp; // Retourner l'ID de l'OTP nouvellement créé
  } catch (error) {
...
exports.sendOtpByEmail = async (req, res) => {
  ...
  // const newOtp = await prisma.otp.findUnique({ where: { id: newOtpId } });
...
exports.verifyOtp = async (req, res) => {
  ...
  try {
    ...
    const userOtp = await prisma.userOtp.findFirst({
      where: { userId },
      orderBy: { createdAt: 'desc' }, // Utilisez l'argument 'orderBy' pour trier par date de création descendant
    });
...
exports.login = async (req, res) => {
  try {
    ...
    if (req.body.email) {
      ...
      check = await prisma.users.findUnique({ where: { number: req.body.number } });
      if (!check) {
        return res.status(401).send("Utilisateur non trouvé !");
      }
